...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpszCmdLine, int iCmdShow) // declarator
{
	...
	// fprintf(gpFile, "PPP: GetMessage() return type : %d", flag);
	while(GetMessage(&msg, NULL, 0, 0))
	{
		fprintf(gpFile, "PPP: In side GetMessage");
...
LRESULT CALLBACK WndProc(HWND hwnd, UINT iMsg, WPARAM wParam, LPARAM lParam) // declarator
{
	...
	{
		case WM_CREATE:
			fprintf(gpFile, "PPP: WM_CREATE: window created successfully...\n");
		...
		case WM_CHAR: // alphabate press message
			switch(wParam)
			{
				case 'F':
				case 'f':
					fprintf(gpFile, "PPP: WM_CHAR: F/f key pressed\n");
					if(!bFullScreen)
					{
						ToggleFullScreen();
						bFullScreen = TRUE;
			...
			break;
		case WM_CLOSE:
			fprintf(gpFile, "PPP: WM_CLOSE: message created\n");
			break;
		case WM_DESTROY:
			printf(gpFile, "PPP: WM_DESTROY: message created\n");
			PostQuitMessage(18);
			break;
		default:
			break;
