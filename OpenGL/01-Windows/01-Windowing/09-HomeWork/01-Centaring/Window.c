...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpszCmdLine, int iCmdShow) // declarator
{
	...
	// Centering using SystemParameterInfo()
	/*
	SystemParametersInfo(SPI_GETDRAGWIDTH, 0, &iScreenWidth, 0);
	SystemParametersInfo(SPI_GETDRAGHEIGHT, 0, &iScreenHeight, 0);
	TCHAR str[255];
	wsprintf(str, TEXT("iScreenWidth = %d and iScreenHeight = %d", iScreenWidth, iScreenHeight));
	MessageBox(NULL, str, TEXT("Debugging ..."), MB_OK);
	*/

	SystemParametersInfo(SPI_GETWORKAREA, 0, &rcMyScreen, 0);
	iScreenWidth = rcMyScreen.right - rcMyScreen.left;
	iScreenHeight = rcMyScreen.bottom - rcMyScreen.top;

	TCHAR str[255];
        wsprintf(str, TEXT("iScreenWidth = %d and iScreenHeight = %d", , iScreenHeight));
        MessageBox(NULL, str, TEXT("Debugging ..."), MB_OK);
